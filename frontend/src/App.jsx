...
import {
  ...
  ThemeProvider, CssBaseline, IconButton, Alert
} from '@mui/material';
    ...
    <ThemeProvider theme={theme}>
      ...
      <Router>
        <Box
          ...
          sx={{
            flexGrow: 1,
            ml: user ? ((user.role === 'Admin' || user.role === 'Team Member') ? '240px' : 0) : 0,
            display: 'flex',
        ...
        >
          {user?.role === 'Team Member' && <Sidebar user={user} onLogout={handleLogout} />}
          {user?.role === 'Admin' && <AdminSidebar user={user} onLogout={handleLogout} />}
                ...
                user ? (
                  ...
                  <Box sx={{ ...fullScreenBg }}>
                    <Box sx={authBox}>
                      ...
                      <TextField fullWidth type="password" label="Password" value={password} onChange={(e) => { setPassword(e.target.value); if (loginError) setLoginError(''); }} margin="normal" />
